...
#Python 3
"""
Same as mv_acquire_continuous.py, but saves results in directory.
...
def acquire_continuous(num_frames, exp_time_us=100, FPS=25, verbosity=0):
    ...
    OUTPUT:
        ...
        ::float:: final_FPS             #final FPS value
    """
    img_list = []
    FPS_list = []
    ...
    for i in range(0,num_frames,1):
        ...
        if fi.isRequestNrValid(requestNr):
            ...
            if pRequest.isOK:
                ...
                if verbosity >= 3: print("The number of unique values in the image is {}".format(np.size(np.unique(arr_rs))))
    ...
    return img_list, FPS_list, final_FPS

def acquire_continuous_and_stack(num_frames, exp_time_us, FPS, savedir, do_plot=False, message=None):
    img_list, FPS_list, final_FPS = acquire_continuous(num_frames, exp_time_us=exp_time_us, FPS=FPS, verbosity=1)
    ...
    savename = os.path.join(savedir, date_string+'.tif')
    img16bit.save(savename)
    ...
    results_dict = dict()
    results_dict['date'] = now.strftime("%Y-%m-%d %H:%M:%S")
    results_dict['number of frames'] = num_frames
    results_dict['exposure_time_us'] = exp_time_us
    results_dict['total_exposure_time_us'] = exp_time_us * num_frames
    results_dict['user_set_FPS'] = FPS
    results_dict['actual_mean_FPS'] = mean_FPS
    results_dict['actual_final_FPS'] = final_FPS
    results_dict['number of frames / actual_final_FPS'] = num_frames / final_FPS
    if message is None:
        message = get_user_input()
    results_dict['message'] = message
    
    results_dict_savename = os.path.join(savedir, date_string+'.json')
    import json
    with open(results_dict_savename, 'w', encoding='utf-8') as f:
...
def get_user_input():
    while True:
        try:
            ...
            print("Error encountered. Let's try again...")
            continue
        ...
        while True:
            try:
                ...
                print(message)
                confirm = str(input("Is this correct? Enter Y or N: "))
            except:
        ...
        if confirm == "Y":
            ...
            print("You did not indicate your message is correct. Let's try again...")
...
if __name__ == "__main__":
    ...
    acquire_continuous_and_stack(num_frames, exp_time_us, FPS, savedir)
