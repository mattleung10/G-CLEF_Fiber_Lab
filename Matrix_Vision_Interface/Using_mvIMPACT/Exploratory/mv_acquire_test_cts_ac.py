...
def acquire_frame(filename, exptime=0.0, ampgain=0.0, verbosity=0): 
    ...
    ac.exposureTime.write(700)
    ac.acquisitionFrameRateEnable.writeS('1')
    ac.acquisitionFrameRate.write(60)
    
    alg_c = acquire.AnalogControl(pDev)
    alg_c.gainAuto.writeS('Off')
    alg_c.gain.write(0.0)
    ...
    s_normalized = s / len(img_list)
    
    #https://stackoverflow.com/questions/63310083/dynamic-range-bit-depth-in-pils-fromarray-function
    arr16bit = np.array(s_normalized).astype(np.uint16)
    arr16bit = np.multiply(arr16bit,2**4)
    
    from PIL import Image
    arr = Image.fromarray(arr16bit)
    arr.save('test.tif')
