...
#File last modified: March 16, 2022
...
def fit_function_to_img(img, fit_fcn, p0, bounds):
    ...
    params, cov = scipy.optimize.curve_fit(fit_fcn, xdata, Z.ravel(), p0=p0, sigma=Z_uncert.ravel(), bounds=bounds, maxfev=1e4)
...
def fit_gaussian(data, xy_guess=None, clipped_gaussian=False):
    ...
    theta_max = np.pi
    theta_init = 0
    lb_dict = {'x0':0, 'y0':0, 'sigma_x':0.5, 'sigma_y':0.5, 'A':0, 'back':0, 'theta':theta_min}
...
def crop_image_safe(data, x, y, zoom_size=200):
    ...
    x_max = np.min([int(x + zoom_size), data.shape[1]-1])
    y_min = np.max([int(y - zoom_size), 0])
    y_max = np.min([int(y + zoom_size), data.shape[0]-1])
