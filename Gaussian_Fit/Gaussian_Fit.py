...
#February 2022
"""
Functions used to fit a Gaussian to an image, iteratively find the centroid
of an image, crop an image, etc.
"""
...
def fit_gaussian(data, xy_guess=None):
    """
    ...
    theta_max = np.pi
    theta_init = 0
    lb_dict = {'x0':0, 'y0':0, 'sigma_x':0, 'sigma_y':0, 'A':0, 'back':np.min(img), 'theta':theta_min}
    ub_dict = {'x0':img.shape[1], 'y0':img.shape[0], 'sigma_x':np.inf, 'sigma_y':np.inf, 'A':np.inf, 'back':np.inf, 'theta':theta_max}
    p0_dict = {'x0':img.shape[1]/2, 'y0':img.shape[0]/2, 'sigma_x':5, 'sigma_y':5, 'A':np.max(img), 'back':1.1*np.min(img), 'theta':theta_init}
...
def find_centroid_iter(img, cen_tol=5, maxiter=10, do_plot=False):
    ...
    radius = np.min([np.min(img.shape)*0.3, smallest_dist_to_edge]) #initialize the radius value to crop image
    ...
    while True:
        ...
        y_cen = new_y_cen
        smallest_dist_to_edge = np.min([x_cen, img.shape[1]-x_cen, y_cen, img.shape[0]-y_cen]) #smallest distance from centroid to edge of an image
        radius = np.min([0.75*radius, smallest_dist_to_edge]) #reduce radius by 0.75 each time
...
if __name__ == "__main__":
    filename = "Test_Images/220214_1035_Image.jpg"
