...
def process_message(json_dict_og):
    ...
    feature_names = ["source", "MS_ON_OFF", "shaft_dist", "frequency", "ND", "MS"]
...
if __name__ == "__main__":
    images_dir = os.path.join(os.getcwd(), os.pardir, "Images", "20220628_Freq_Vary_Oct")
    ...
    off_df.loc[:,feature] = off_df.loc[:,feature] * 40/24 #Lego gear ratio
    off_freq = off_df[feature].to_numpy()
    off_snr = off_df['mean/std'].to_numpy()
    off_unique_freq, off_ms_means, off_ms_stds = get_mean_std_points(off_df, feature=feature, metric='mean/std')
    
    on_df = df[df['MS_ON_OFF'] == "MS ON"]
    on_df.loc[:,feature] = on_df.loc[:,feature] * 40/24 #Lego gear ratio
    on_freq = on_df[feature].to_numpy()
    on_snr = on_df['mean/std'].to_numpy()
    on_unique_freq, on_ms_means, on_ms_stds = get_mean_std_points(on_df, feature=feature, metric='mean/std')
    ...
    capsize = 6
    pltcolor = 'dimgrey'
    ax[0].scatter(on_freq, on_snr, color='tab:blue', s=25, linewidth=mew, edgecolor='k', label='Scrambler ON', zorder=5)
    ax[0].errorbar(on_unique_freq, on_ms_means, yerr=on_ms_stds, fmt='-', markersize=ms, ecolor=pltcolor, elinewidth=1, color=pltcolor, capsize=capsize, markeredgewidth=mew, markeredgecolor='k', zorder=4)
    ax[0].set_ylabel('Signal-to-Noise Ratio')
    curr_handles, curr_labels = ax[0].get_legend_handles_labels()
    handles += curr_handles
    labels += curr_labels
    
    ax[1].scatter(off_freq, off_snr, color='tab:orange', s=25, linewidth=mew, edgecolor='k', label='Scrambler OFF', zorder=5)
    ax[1].errorbar(off_unique_freq, off_ms_means, yerr=off_ms_stds, fmt='-', markersize=ms, ecolor=pltcolor, elinewidth=1, color=pltcolor, capsize=capsize, markeredgewidth=mew, markeredgecolor='k', zorder=4, label='Average')
