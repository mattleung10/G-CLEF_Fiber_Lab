...
if __name__ == "__main__":
    images_dir = os.path.join(os.getcwd(), os.pardir, "Images", "20220603_Tot_Exp_Const")
    ...
    feature = "exposure_time_us"
    off_df = df[df['MS_ON_OFF'] == "MS OFF"]
    off_exp_time_ms = off_df[feature].to_numpy() / 1000
    off_snr = off_df['mean/std'].to_numpy()
    off_unique_exp_time, off_ms_means, off_ms_stds = get_mean_std_points(off_df, feature=feature, metric='mean/std')
    off_unique_exp_time_ms = off_unique_exp_time / 1000
    
    on_df = df[df['MS_ON_OFF'] == "MS ON"]
    on_exp_time_ms = on_df[feature].to_numpy() / 1000
    on_snr = on_df['mean/std'].to_numpy()
    on_unique_exp_time, on_ms_means, on_ms_stds = get_mean_std_points(on_df, feature=feature, metric='mean/std')
    on_unique_exp_time_ms = on_unique_exp_time / 1000
    ...
    capsize = 6
    pltcolor = 'dimgrey'
    ax[0].scatter(on_exp_time_ms, on_snr, color='tab:blue', s=25, linewidth=mew, edgecolor='k', label='Scrambler ON', zorder=5)
    ax[0].errorbar(on_unique_exp_time_ms, on_ms_means, yerr=on_ms_stds, fmt='-', markersize=ms, ecolor=pltcolor, elinewidth=1, color=pltcolor, capsize=capsize, markeredgewidth=mew, markeredgecolor='k', zorder=4)
    ax[0].set_xlabel('Exposure Time [ms]')
    ax[0].set_ylabel('Signal-to-Noise Ratio')
    ax[0].set_xscale('log')
    secax0 = ax[0].secondary_xaxis('top', functions=(exp_to_frames, frames_to_exp))
    secax0.set_xlabel('Number of Stacked Frames')
    
    curr_handles, curr_labels = ax[0].get_legend_handles_labels()
    handles += curr_handles
    labels += curr_labels
    
    ax[1].scatter(off_exp_time_ms, off_snr, color='tab:orange', s=25, linewidth=mew, edgecolor='k', label='Scrambler OFF', zorder=5)
    ax[1].errorbar(off_unique_exp_time_ms, off_ms_means, yerr=off_ms_stds, fmt='-', markersize=ms, ecolor=pltcolor, elinewidth=1, color=pltcolor, capsize=capsize, markeredgewidth=mew, markeredgecolor='k', zorder=4, label='Average')
    ax[1].set_xlabel('Exposure Time [ms]')
    ax[1].set_ylabel('Signal-to-Noise Ratio')
    ax[1].set_xscale('log')
    secax1 = ax[1].secondary_xaxis('top', functions=(exp_to_frames, frames_to_exp))
    secax1.set_xlabel('Number of Stacked Frames')
