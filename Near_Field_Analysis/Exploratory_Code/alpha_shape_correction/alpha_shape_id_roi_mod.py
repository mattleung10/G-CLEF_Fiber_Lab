...
def hull_mask_from_alpha_shape(img, alpha=100, CannyMaxFactor=1/4, CannyMinFactor=3/4, is12bit=True, verbosity=0):
    """
    ...
    PROCEDURE:
        ...
        4) Create a binary mask of all pixels inside the alpha shape outer boundary
    """    
...
if __name__ == "__main__":
    ...
    filename = os.path.join(images_dir, '20220602_164917.tif')
    filename = os.path.join(images_dir, '20220602_174204.tif')
    json_filename = os.path.splitext(filename)[0] + ".json" #the filename of the associated json file to the image
    json_dict = load_json(json_filename)
    ...
    img = subtract_dark_and_bias(img, json_dict["exposure_time_us"], dict_of_dark_images, min_exp_time_us)
    
    hull_mask = hull_mask_from_alpha_shape(img, alpha=100, CannyMaxFactor=1/4, CannyMinFactor=3/4, is12bit=True, verbosity=2)
